  ...
  <header class="sticky top-0 z-50 border-b border-stone-800/50 bg-stone-950/80 backdrop-blur-md">
    <nav class="mx-auto flex max-w-7xl items-center justify-between px-6 py-4" aria-label="Navigation principale">
      ...
      <ul class="hidden items-center gap-8 md:flex">
        <li>
          <a :href="mailtoHref"
            class="text-stone-400 transition-colors hover:text-accent flex items-center gap-1 text-sm tracking-wider uppercase"
      ...
      <!-- Mobile menu button -->
      <button class="text-stone-400 md:hidden" :aria-expanded="mobileMenuOpen" aria-controls="mobile-menu"
        aria-label="Menu de navigation" @click="mobileMenuOpen = !mobileMenuOpen">
        <svg v-if="!mobileMenuOpen" xmlns="http://www.w3.org/2000/svg" class="size-6" fill="none" viewBox="0 0 24 24"
    ...
    <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="-translate-y-2 opacity-0"
      ...
      <div v-if="mobileMenuOpen" id="mobile-menu" ref="mobileMenuEl"
        class="border-t border-stone-800/50 bg-stone-950/95 backdrop-blur-md md:hidden" role="dialog" aria-modal="true"
        aria-label="Menu de navigation" @keydown="onMobileKeydown">
