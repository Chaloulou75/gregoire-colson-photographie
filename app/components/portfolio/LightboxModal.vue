...
<template>
  <Teleport to="body">
    <div ref="lightboxEl" class="fixed inset-0 z-50 flex items-center justify-center bg-stone-950/95 backdrop-blur-sm"
      role="dialog" aria-modal="true" aria-label="Galerie photo" @click.self="emit('close')">
      <!-- Close button -->
      <button class="absolute right-4 top-4 z-10 rounded-full p-2 text-stone-400 transition-colors hover:text-stone-100"
      ...
      <button v-if="hasPrev"
        ...
        aria-label="Photo précédente" @click="prev">
        <svg xmlns="http://www.w3.org/2000/svg" class="size-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"
          stroke-width="1.5">
      ...
      <!-- Image -->
      <img :src="images[currentIndex]" :alt="`Photo ${currentIndex + 1} sur ${images.length}`"
        class="max-h-[85vh] max-w-[90vw] object-contain">

      <!-- Next -->
      <button v-if="hasNext"
        class="absolute right-4 z-10 rounded-full p-2 text-stone-400 transition-colors hover:text-stone-100"
        aria-label="Photo suivante" @click="next">
        <svg xmlns="http://www.w3.org/2000/svg" class="size-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"
          stroke-width="1.5">
